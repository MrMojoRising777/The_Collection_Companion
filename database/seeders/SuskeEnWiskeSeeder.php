...
class SuskeEnWiskeSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $seriesMap = Serie::all()
            ->mapWithKeys(fn (Serie $serie): array => [
                $serie->abbreviation => $serie->id,
            ])
            ->toArray();
        ...
        $crawler->filter('table.wikitable tbody tr')
            ->each(function (Crawler $row, int $rowIndex) use ($seriesMap): void {
                if ($rowIndex === 0) {
                    return;
                };
                ...
                for ($i = 3; $i < $cells->count(); $i++) {
                    ...
                    $abbr = $this->getSeriesAbbreviationByIndex(index: $i);

                    if (! $abbr || ! isset($seriesMap[$abbr])) {
                        continue;
                    };
