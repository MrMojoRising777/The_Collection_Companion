...
@section('content')
  <div class="container">
    <h1 class="h1 text-center mb-3 suske_wiske_font">Alle Series</h1>
    <blockquote>
      <i>Bekijk en doorzoek ALLE series in de app. Beslis hier of je de serie wil verzamellen, deze verschijnt dan in jouw collectie.</i>
    </blockquote>
          ...
          <tbody>
            @foreach ($series as $serie)
              <tr onclick="window.location='{{ route('series.show', $serie->id) }}';" style="cursor:pointer;">
                ...
                <td>{{ $serie->abbreviation }}</td>
                <td>{{ $serie->name }}</td>
                <td>{{ $serie->period }}</td>
                <td>{{ $serie->albums_count }}</td>
              </tr>
            @endforeach
          </tbody>
        </table>
            ...
            <div class="col-lg-4 col-md-6">
              <div class="card mb-4">
                <div class="row no-gutters">
                  ...
                  <div class="col-md-6">
                    <div class="card-body">
                      <h5 class="card-title">{{ $serie->abbreviation }} - {{ $serie->name }}</h5>
                      <p class="card-text">{{ $serie->period }}</p>
                      <p class="card-text">Aantal strips {{ $serie->albums_count }}</p>
