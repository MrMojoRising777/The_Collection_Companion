...
@section('content')
  <div class="container">
    <h1 class="h1 text-center mb-3 suske_wiske_font">Jouw albums</h1>

    <blockquote>
      <i>Bekijk en doorzoek JOUW collectie in de app. Bekijk je SERIES of ALBUMS.</i>
    </blockquote>
      ...
      <div class="row">
        <div class="col-md-4 mb-3">
          @if(Route::currentRouteName() !== 'collection.albums.index')
            <a href="{{ route('collection.albums.index') }}" class="btn btn-success">
              Collectie
            </a>
          @endif
          @if (Route::currentRouteName() !== 'collection.albums.favorites')
            <a href="{{ route('collection.albums.favorites') }}" class="btn btn-warning">
              Favorieten
            </a>
          @endif
          @if (Route::currentRouteName() !== 'collection.albums.first_prints')
            <a href="{{ route('collection.albums.first_prints') }}" class="btn btn-secondary">
              Eerste drukken
            </a>
          @endif
        </div>
          ...
          <table class="table">
            ...
            <tbody>
              @foreach ($collection as $collected)
                <tr onclick="window.location='{{ route('collection.albums.show', $collected->album_id) }}';" style="cursor:pointer;">
                  <td class="d-flex">

                    <form method="POST" action="{{ route('collection.albums.removeFromCollection', $collected->album_id) }}">
                      @csrf
                      @method('DELETE')
                  ...
                  <td>
                    <a href="{{ route('collection.albums.show', $collected->album_id) }}" class="btn btn-info">Bekijk</a>
                    <a href="{{ route('collection.albums.edit', $collected->album_id) }}" class="btn btn-warning">Wijzig</a>
                  </td>
                </tr>
              @endforeach
            </tbody>
                  ...
                  <div class="row no-gutters">
                    ...
                    <div class="col-md-6">
                      <div class="card-body">
                        ...
                        <a href="{{ route('collection.albums.show', $collected->album_id) }}" class="btn btn-info">View</a>
